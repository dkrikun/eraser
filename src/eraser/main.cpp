...
{
        eraser::scoped_lock( jvmti, eraser::agent::instance()->monitor_ );
        eraser::logger::instance()->level(0) << INFO << std::endl;
...
#				if defined( ERASER_DEBUG )
        	    ("obj-filter", po::value<std::string>()->default_value("Linc\\/Cell;")
        	    		, "regex to match classes (for field watches)")
				("thread-filter", po::value<std::string>()
						->default_value
						(
						"Linc\\/Worker;"
						"|Linc\\/SynchWorker;"
						"|Linc\\/MethodSynchWorker;"
						"|Linc\\/Cell;"
						)
...
#				else
				("obj-filter", po::value<std::string>()->default_value("\\w+(\\w+|/)*\\w+")
						, "regex to match classes (for field watches)")
				("thread-filter", po::value<std::string>()->default_value("\\w+(\\w+|/)*\\w+")
						, "regex to match classes for monitor enter/exit instrumentation")
#				endif
        	;

        	eraser::logger::instance()->set_curr_level(0);
        	po::variables_map vm;
        	std::vector<std::string> split_args = po::split_unix( options, "," );
#			if defined( SHOW_PARSED_ARGS )
        	std::cout << "Parsed args:\n\t";
        	std::copy( split_args.begin(), split_args.end()
...
#			endif
        	...
        	eraser::agent::instance()->filter_regex_ = vm["obj-filter"].as<std::string>();
        	eraser::agent::instance()->thread_filter_regex_ = vm["thread-filter"].as<std::string>();

        	eraser::logger::instance()->set_curr_level( vm["log-level"].as<unsigned int>() );
        }
        eraser::logger::instance()->level(4) << "regex_filter= " << eraser::agent::instance()->filter_regex_ << std::endl;
        eraser::logger::instance()->level(4) << "tregex_filter= " << eraser::agent::instance()->thread_filter_regex_ << std::endl;
...
{
	eraser::logger::instance()->level(1) << INFO << std::endl;
}
